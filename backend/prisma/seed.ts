...
async function main(){
	...
	if (invCount === 0) {
		await prisma.inventoryItem.createMany({ data: [
			{ name: 'Sơn chống rỉ', uom: 'lit', qty_on_hand: 50, reorder_point: 10, unit_price: 300000 },
			{ name: 'Đinh tán', uom: 'pcs', qty_on_hand: 1000, reorder_point: 200, unit_price: 10000 },
			{ name: 'Ron cao su', uom: 'pcs', qty_on_hand: 500, reorder_point: 100, unit_price: 20000 }
		] });
	}
}
